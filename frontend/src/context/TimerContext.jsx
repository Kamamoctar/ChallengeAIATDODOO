...
const WORK_SECS        = 25 * 60   // 25 min focus (défaut)
...
export function TimerProvider({ children }) {
  ...
  // Durée d'une phase de travail = pomodoroMin (configurable), sinon WORK_SECS par défaut.
  const workSecs = (pomodoroMin || 25) * 60
  ...
  function start(task) {
    ...
    const t = {
      ...
      phase: 'work', pomodoroCount: 0, pomodoros: 0,
  ...
  async function stop() {
    ...
    if (!navigator.onLine) {
      ...
      try {
        ...
        // Comparaison au temps estimé de la tâche (allocated_hours) + compteur pomodoros
        let extra = ''
        const doneP = saved.pomodoros || saved.pomodoroCount || 0
        if (doneP) extra += ` · ${doneP} pomodoro${doneP > 1 ? 's' : ''}`
  ...
  return (
    <TimerContext.Provider value={{
      ...
      pomodoros: timer?.pomodoros || timer?.pomodoroCount || 0,
